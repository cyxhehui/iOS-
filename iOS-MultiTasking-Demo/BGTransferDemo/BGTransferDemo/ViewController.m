...
@property (nonatomic, strong) NSMutableArray<FileDownloadInfo*> *arrFileDownloadData;
...
- (void)viewDidLoad
{
    ...
    self.databaseManager = [[DBManager alloc]initDatabaseWithPath:self.docDirectoryURL.path];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (fdi.status == HTFileTransferStateNone) {
    ...
    else if(fdi.status == HTFileTransferStateTransfering || fdi.status == HTFileTransferStatePaused){
    ...
    else if(fdi.status == HTFileTransferStateDone){
        progressView.hidden = YES;
        stopButton.enabled = NO;
